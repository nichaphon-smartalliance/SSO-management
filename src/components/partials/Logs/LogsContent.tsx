...
import { SEVERITY_OPTIONS, SEVERITY_CONFIG } from "./Logs.config";
...
export function LogsContent() {
  ...
  const statCards = (Object.keys(SEVERITY_CONFIG) as Severity[]).map((key) => ({
    ...SEVERITY_CONFIG[key],
    key,
    value: mockLogs.filter((l) => l.severity === key).length,
  }));
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((log) => {
                  const sev = SEVERITY_CONFIG[log.severity as Severity];
                  return (
