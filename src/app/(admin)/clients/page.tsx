...
export default function ClientsPage() {
  ...
  const statusBadge = (status: string) => {
    ...
    if (status === "expired") return <Badge variant="warning">หมดอายุ</Badge>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">จัดการระบบงาน (Client)</h1>
          <p className="text-sm text-slate-500 mt-1">จัดการ OAuth 2.0 Clients ทั้งหมด</p>
        </div>
        <Button>
          <Plus className="w-4 h-4" /> เพิ่มระบบงาน
        </Button>
      </div>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="ค้นหาระบบงาน..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <select
