...
function statusBadge(status: string) {
  if (status === "pending") return <Badge variant="warning">รอดำเนินการ</Badge>;
...
export default function RequestsPage() {
  ...
  const RequestTable = ({ items }: { items: typeof mockRequests }) => (
    <div className="overflow-x-auto">
      ...
      {items.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">ไม่พบข้อมูล</div>}
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">คำขอใช้งานระบบ</h1>
        <p className="text-sm text-slate-500 mt-1">จัดการคำขอเข้าถึงระบบงานต่างๆ</p>
      </div>

      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
        <Input placeholder="ค้นหาคำขอ..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
